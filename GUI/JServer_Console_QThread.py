...
class JConsole_QThread(QThread):
    ...
    def get_process_id(self):
        for process_element in psutil.process_iter(['pid', 'name']):
            try:
                if self.key_command == 'roscore':
                    if self.key_command in process_element.info['name']:
                        print('roscore')
                        return process_element.pid
                elif self.value_command in process_element.info['name']:
                    print('ros', process_element.pid)
                    return process_element.pid
                else:
