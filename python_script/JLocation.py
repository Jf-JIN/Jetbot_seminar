...
class JApril_Tag_Info():
    ...
    # 赋值距离参数
    def set_distance(self, distance_list: list) -> None:
        # 如果输入不是元组或是列表，亦或元组/列表中的元素不是浮点数或字符串，亦或其元素个数不为3，即非(x,y,z)，则触发报错
        ...
        if not distance_list[0] and not distance_list[1] and not distance_list[2]:
            return
        self.__distance.set_x(float(distance_list[0]))
        self.__distance.set_y(float(distance_list[1]))
        self.__distance.set_z(float(distance_list[2]))

    # 赋值方向参数
    def set_orientation(self, orientation_list: list) -> None:
        # 如果输入不是元组或是列表，亦或元组/列表中的元素不是浮点数或字符串，则触发报错
        ...
        self.__orientation.set_y(float(orientation_list[1]))
        self.__orientation.set_z(float(orientation_list[2]))
    # 赋值id参数
...
class JImu_Info():
    ...
    # 赋值方向
    def set_orientation(self, orientation_list: list):
    ...
    def set_velocity(self, velocity_list: list) -> None:
        ...
        self.__imu_velocity.set_y(float(velocity_list[1]))
        self.__imu_velocity.set_z(float(velocity_list[2]))

    # 赋值角速度参数
    def set_angular_velocity(self, angular_velocity_list: list) -> None:
        ...
        self.__imu_angular_velocity.set_y(float(angular_velocity_list[1]))
        self.__imu_angular_velocity.set_z(float(angular_velocity_list[2]))

    # 赋值加速度参数
    def set_acceleration(self, acceleration_list: list) -> None:
        ...
        self.__imu_acceleration.set_y(float(acceleration_list[1]))
        self.__imu_acceleration.set_z(float(acceleration_list[2]))

    # 赋值角加速度参数
    def set_angular_acceleration(self, angular_acceleration_list: list) -> None:
        ...
        self.__imu_angular_acceleration.set_y(float(angular_acceleration_list[1]))
        self.__imu_angular_acceleration.set_z(float(angular_acceleration_list[2]))

    # 赋值磁场参数
    def set_magnetic_field(self, magnetic_field_list: list) -> None:
        ...
        self.__magnetic_field.set_y(float(magnetic_field_list[1]))
        self.__magnetic_field.set_z(float(magnetic_field_list[2]))
...
class JLocation():
    ...
    # 获取左侧位置信息
    @ property
    def left_list(self) -> list:
        return self.__left_list

    # 获取右侧信息
    @ property
    def right_list(self) -> list:
