...
logger_dict = JLog('Socket', '文本端口')
info = logger_dict['info']
log_error = logger_dict['error']

# def write_error_info(text):
#     log_folder_path = os.path.join(os.path.dirname(__file__), 'log')
#     if os.path.exists(log_folder_path):
#         os.mkdir(log_folder_path)
#     log_path = os.path.join(log_folder_path, f'LOG_{int(time.time())}.txt')
...
class Client_Console_QThread(QThread):
    ...
    def __init__(self, ip: str) -> None:
        ...
        info('[创建Socket] - {self.formatted_time}')
        self.ip = ip
        ...
        # [WinError 10057] 由于套接字没有连接并且(当使用一个 sendto 调用发送数据报套接字时)没有提供地址, 发送或接收数据的请求没有被接受。
            ...
            e = traceback.format_exc()
            text = f'{e}'
            self.signal_error_output.emit(text)
            log_error(text)
    ...
    def read_and_analyse_data(self, data: dict):
        try:
            ...
            if 'JPort' in data:
                info(f'与 {self.server_address} 成功建立连接')
                self.signal_connected_port.emit(str(data['JPort']))
                info(f'客户端端口: {data["JPort"]}')
                return
...
class Server_Console_QThread(QThread):
    ...
    def __init__(self, ip: str) -> None:
        ...
        try:
            ...
            info(f'服务器在 {self.ip}:{DEFAULT_PORT} 等待连接...')
        except Exception as e:
            e = traceback.format_exc()
    ...
    def clients_list_remove(self, client: list) -> None:
        ...
        info(f'[当前客户端列表]\t {self.clients_list}')
        ...
        while self.flag_running:
            try:
                ...
                if client_socket:
                    ...
                    info(f'[+]: 接受来自 {client_address} 的连接')
                    info(f'当前已连接客户端数量: {len(self.clients_list)}')
                    # 接收客户端信息
...
class Server_Handle_Console_QThread(QThread):
    ...
    def read_and_analyse_data(self, data: dict):
        try:
            info(f'{type(data)}\t{repr(data)}')
            data = json.loads(data)
            if 'close' in data:
                info(f'客户端 {self.client_address} 请求断开连接')
                self.flag_running = False
            elif 'server_close' in data:
                info(f'由 {self.client_address} 发出关闭服务器的请求 - {self.formatted_time}')
    ...
    def run(self) -> None:
        try:
            ...
            info(f'客户端 {self.client_address} 已断开连接')
            ...
            info(f'当前已连接客户端数量: {len(self.parent_obj.clients_list)}')
            info(f'[-]: 与 {self.client_address} 的连接已关闭')
